...
uses
  ...
  Dialogs, LikeList, DB, dxBar, FIBDataSet, pFIBDataSet, FIBDatabase,
...
type
  TfrmAdminProg = class(TfrmLikeList)
...
begin
 FName := 'Администрирование программы';
 inherited
end;
...
procedure TfrmAdminProg.Run(ID : Integer);
begin
 OpenAll;
 Show
end;
...
begin
 ...
 fibqLocalAux.SQL.Clear;
 fibqLocalAux.SQL.Add('update PROGRAM_GROUPS set ROLE_NAME='''+S+''' where (ID='+ID+')');
 fibqLocalAux.ExecQuery;
 trMain.CommitRetaining;
 fibqLocalAux.Close;
 fibqLocalAux.Free;
 fibdsProgramGroups.Refresh;
 fibdsMainList.Refresh
end;

procedure TfrmAdminProg.PopupMenu1Popup(Sender: TObject);
begin
 N1.Enabled := ((tlGroups.IsActive) and (fibdsProgramGroupsROLE_COUNT.AsInteger=0)) or
               ((tvMainList.IsControlFocused) and (fibdsMainListROLE_COUNT.AsInteger=0))
end;
...
begin
 ...
 If (MessageDlg('Хотите переместить группу ['+tvMainList.DataController.Values[tvMainList.DataController.FocusedRecordIndex,tvMainListGROUP_NAME.Index]+'] в группу ['+tlGroups.FocusedNode.Values[tlGroupsNAME.ItemIndex]+']',mtConfirmation,mbOKCancel,0)=mrOK) then
  Begin
   ...
   fibqLocalAux.Transaction := fibdsMainList.Transaction;
   fibqLocalAux.SQL.Clear;
   fibqLocalAux.SQL.Add('update PROGRAM_GROUPS set PARENT_ID='+VarAsType(tlGroups.FocusedNode.Values[tlGroupsID.ItemIndex],varString)+' where (ID='+VarAsType(AGridView.DataController.Values[AGridView.DataController.FocusedRecordIndex,tvMainListID.Index],varString)+')');
   fibqLocalAux.ExecQuery;
   trMain.CommitRetaining;
   fibqLocalAux.Close;
   fibqLocalAux.Free;
...
begin
 ...
 fibqLocalAux.SQL.Clear;
 If (fibdsMainRoleRightsSELECT_RIGHT.OldValue<>fibdsMainRoleRightsSELECT_RIGHT.NewValue) then
  Begin
   ...
   fibqLocalAux.ExecQuery;
  End;
 If (fibdsMainRoleRightsINSERT_RIGHT.OldValue<>fibdsMainRoleRightsINSERT_RIGHT.NewValue) then
  Begin
   fibqLocalAux.SQL.Clear;
   ...
   fibqLocalAux.ExecQuery;
  End;
 If (fibdsMainRoleRightsUPDATE_RIGHT.OldValue<>fibdsMainRoleRightsUPDATE_RIGHT.NewValue) then
  Begin
   fibqLocalAux.SQL.Clear;
   ...
   fibqLocalAux.ExecQuery;
  End;
 If (fibdsMainRoleRightsDELETE_RIGHT.OldValue<>fibdsMainRoleRightsDELETE_RIGHT.NewValue) then
  Begin
   fibqLocalAux.SQL.Clear;
   ...
   fibqLocalAux.ExecQuery;
  End;
 If (fibdsMainRoleRightsREFERENSE_RIGHT.OldValue<>fibdsMainRoleRightsREFERENSE_RIGHT.NewValue) then
  Begin
   fibqLocalAux.SQL.Clear;
   ...
   fibqLocalAux.ExecQuery;
  End;
 If (fibdsMainRoleRightsEXECUTE_RIGHT.OldValue<>fibdsMainRoleRightsEXECUTE_RIGHT.NewValue) then
  Begin
   fibqLocalAux.SQL.Clear;
