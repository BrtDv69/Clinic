...
type
  ...
  InstitutionRec = Record
   Name : String;
   ChiefSignName  : String;
   ChiefJob: String;
  end;
...
var
 ...
 clrReadOnly : TColor = clMedGray;
 clrNotValid : TColor = clGray;

function ReportGetSenderName(Sender: TComponent): String;
procedure GetReport(Database: TFIBDatabase; Transaction: TFIBTransaction; ReportID: Integer; ReportName: String);
...
begin
 ...
 fibqLocalAux.SQL.Add('select R.REPORT from REPORTS R where (R.ID='+IntToStr(ReportID)+')');
 fibqLocalAux.ExecQuery;
 If (fibqLocalAux.RecordCount>0) and Not(fibqLocalAux.Fields[0].IsNull) then
  Begin
   MS := TMemoryStream.Create;
   try
    fibqLocalAux.Fields[0].SaveToStream(MS);
    frmDM.frxReport.LoadFromStream(MS);
   finally
    MS.Free
   end
  End;
 fibqLocalAux.Close;
 fibqLocalAux.Free;
 frmDM.frxReport.FileName := ReportName
end;
...
procedure InitializeConst(Database: TFIBDatabase; Transaction: TFIBTransaction);
 var fibqLocalAux : TpFIBQuery;
     Started : Boolean;
begin
 User.LogInName := 'Мавлютов А.Х.';
 User.SignName := 'Мавлютов А.Х.';
 User.PrintAdmin := True;
 fibqLocalAux := TpFIBQuery.Create(Nil);
 fibqLocalAux.Database := Database;
 fibqLocalAux.Transaction := Transaction;
